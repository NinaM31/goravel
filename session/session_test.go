...
func TestSession_InitSession(t *testing.T) {
	...
	if sessKind != reflect.ValueOf(sm).Kind() {
		t.Error("wrong kind returned testing cookie sessio. Expected:", reflect.ValueOf(sm).Kind(), "and got", sessKind)
	}
	if sessType != reflect.ValueOf(sm).Type() {
		t.Error("wrong kind returned testing cookie sessio. Expected:", reflect.ValueOf(sm).Type(), "and got", sessType)
	}
}
